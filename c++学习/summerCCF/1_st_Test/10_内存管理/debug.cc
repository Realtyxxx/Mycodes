...
bool alloc(int x) {
  ...
  // cout << "findAllocPoint[" << i << "] = " << findAllocPoint[i] << endl;
  ...
  for (free_begin = 0; free_begin < memorySize; free_begin++) {
    if (!memory[free_begin]) {
      ...
      if (!memory[findD]) {
        ...
        cout << "findAllocPoint[" << findD << "] = " << findAllocPoint[findD] << endl;
        cout << "num :" << num << endl;
        int h_idx = findD;
        int t_idx = used[num].tailidx;
        ...
        cout << "len :" << len << endl;
        cout << "free_begin :" << free_begin << endl;
        free_begin += len - 1;
        cout << "free_begin :" << free_begin << endl;
        // because it would have a self-incl right after then ,so not to skip a place;
      } else {
        // if the remain memory was all-not-used then run here;
        cout << "Done" << endl;
        break;
      }
    }
