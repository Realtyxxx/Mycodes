...
char* getl(char* buffer, int n, FILE* fid) {
  ...
  while ((ptr != NULL) && (buffer[0] == '#')) ptr = fgets(buffer, n, fid);
...
Graph<string>* createGraph(FILE* fid) {
  char buffer[LINELEN +
              1];  // Linebuffer for reading 将从文件读取内容缓存到buffer

  int i, v1, v2, dist;

  /*【读取顶点个数】 */

  if (getl(buffer, LINELEN, fid) ==
      NULL)  // Unable to get number of vertices无法读取节点数
  ...
  if (getl(buffer, LINELEN, fid) ==
      NULL)  // Unable to get graph type 读取图类型错误
  ...
  for (int m = 0; m < num_edge; m++) {
    ...
    tvalue =
        sbuff.substr(loc1 + 1, loc2 - loc1 - 1);  //读取第二个顶点信息到tvalue
