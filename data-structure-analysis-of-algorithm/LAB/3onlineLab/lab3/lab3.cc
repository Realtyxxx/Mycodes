...
class BinTree  //二叉树类
{
 private:
  ...
  void preOrder(
      BinNode<E>* tmp,
      void (*visit)(BinNode<E>* node))  //先序遍历，void(*visit)(BinNode<E>*node)为一个函数指针参数，用visit代替传进来的函数，在遍历函数中使用传进来的函数功能
  {
    if (tmp == NULL) return;
    if (tmp != NULL) (*visit)(tmp);
    if (tmp->left()) preOrder(tmp->left(), (*visit));
    if (tmp->right()) preOrder(tmp->right(), (*visit));
  }
  void inOrder(
      BinNode<E>* tmp,
      void (*visit)(BinNode<E>* node))  //中序遍历，void(*visit)(BinNode<E>*node)为一个函数指针参数，用visit代替传进来的函数，在遍历函数中使用传进来的函数功能
  {
    if (tmp == NULL) return;
  ...
  void postOrder(
      BinNode<E>* tmp,
      void (*visit)(BinNode<E>* node))  //后序遍历，void(*visit)(BinNode<E>*node)为一个函数指针参数，用visit代替传进来的函数，在遍历函数中使用传进来的函数功能
  {
    if (tmp == NULL) return;
  ...
  void LevelOrderTranverse(
      BinNode<E>* tmp,
      void (*visit)(BinNode<E>* node))  //层次遍历，void(*visit)(BinNode<E>*node)为一个函数指针参数，用visit代替传进来的函数，在遍历函数中使用传进来的函数功能
  {
    if (tmp == NULL) return;
    queue<BinNode<E>*> q;
    q.push(tmp);
  ...
  {
    return max(0, BinTreeHeight(tmp) - 1);
  }
  ...
  {
    if (tmp != NULL)
      return max(1 + BinTreeHeight(tmp->left()), 1 + BinTreeHeight(tmp->right()));
    else
      return 0;
  }
  ...
  {
    if (tmp == NULL) return 0;
    return (tmp->isLeaf() ? 1 : 0) + BinTreeLeafs(tmp->left()) + BinTreeLeafs(tmp->right());
  }
...
{
  std::cout << "现在进入构建二叉树程序......" << std::endl;
  std::cout << "请输入二叉树有多少个结点(空结点也计算其中)" << std::endl;
  int n = 0;
  cin >> n;
  std::cout << "请按preorder顺序输入，遇到NULL请输入'/',用空格隔开或者回车隔开均可以" << std::endl;
...
int main() {
  ...
  std::cout << std::endl;
  std::cout << "5:记录树的深度:";
  std::cout << BT->BinTreeDepth() << std::endl;
  std::cout << "6:记录树的高度:";
  std::cout << BT->BinTreeHeight() << std::endl;
  std::cout << "7:统计结点:";
  std::cout << BT->BinTreeNodes() << std::endl;
  std::cout << "8:统计叶子结点:";
  std::cout << BT->BinTreeLeafs() << std::endl;
  std::cout << "9:查找" << strfind << ":";
